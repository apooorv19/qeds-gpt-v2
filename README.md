...
RQ worker service
  |
  v
ChromaDB files + Groq + HuggingFace embeddings
```

### What Can Run on Streamlit Community Cloud

Streamlit Community Cloud is suitable for the frontend only:
...
```toml
QEDS_API_URL = "https://your-fastapi-backend.example.com/api/v1"
```

### What Should Not Run on Streamlit Community Cloud

Do not try to run the entire async architecture inside Streamlit Community Cloud:

- It does not run your `docker-compose.yml`.
- It is not meant to host Redis as a sidecar service.
- It is not meant to run a persistent RQ worker next to the app.
- It hibernates inactive apps, which is bad for always-on queue workers.
- It has resource limits that make the full RAG backend fragile.

### Suitable Free or Low-Cost Hosting Options

Free tiers change often, so verify limits before final deployment.

| Component | Good Options |
|---|---|
| Streamlit frontend | Streamlit Community Cloud, Render web service, Koyeb web service |
| FastAPI backend | Render web service, Koyeb service, Railway service, Fly.io app |
| Redis | Upstash Redis free tier, Render Key Value free tier, Redis Cloud free tier |
| RQ worker | Render background worker, Koyeb worker/service, Railway service, Fly.io machine |

For a portfolio deployment, a practical setup is:

- Streamlit Community Cloud for frontend.
- Render or Koyeb for FastAPI.
- Upstash Redis for managed Redis.
- Render/Koyeb/Railway/Fly.io for the worker.

For the simplest production-like deployment, use one platform that supports both web services and workers, then connect it to managed Redis.

---

## Deployment Notes

Yes, for a portfolio or recruiter-facing demo, keeping Streamlit Community Cloud for the frontend is reasonable.

Reasons:

- It is simple to connect to GitHub.
- It gives you a public URL quickly.
- It is designed specifically for Streamlit apps.
- Your frontend is lightweight now because heavy RAG execution moved to FastAPI and the worker.

Consider migrating the frontend to the same platform as the backend if:

- You want one deployment dashboard.
- You need private networking between frontend and backend.
- You need custom Docker-based deployment for every service.
- You want tighter control over uptime, regions, and secrets.
